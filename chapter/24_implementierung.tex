...
Die generierten Sourcen der XSD-Dateien werden in ein separates Verzeichnis namens \enquote{generated} erzeugt (siehe \ref{lst:buildhelperplugin}). 
...
Um JAXB mit \gls{Maven} zu nutzen, muss das \gls{Maven}-Plugin \enquote{maven-jaxb2-plugin} in die \gls{pom} eingetragen werden. Listing \ref{lst:jaxbplugin} zeigt den XML-Auschnitt aus der \gls{pom}. \index{Maven!pom.xml}
...
Das Listing \ref{lst:getexternalids} zeigt den SQL-Query der Lösung des Problems.
...
Zur Erklärung des Listings \ref{lst:getexternalids}:
...
Um zu verstehen, wie diese Abfrage in der Applikation im Kontext verwendet wird, sei auf das Sequenzdiagramm \ref{fig:sequenzdiagrammsimplequery} verwiesen. 
