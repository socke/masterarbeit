...
Um beide Technologien miteinander zu vergleichen wurde nach der REST-Implementierung noch ein SOAP-Webservice implementiert. 

Das \autoref{kap:anhang_soap_webservice} erläutert die Implementierung des SOAP-Webservices basierend auf der Business Logik und des vorhandenen Modells. 

Das Ergebnis ist, dass sehr einfach auf der vorhandenen Implementierung ein weiterer Webservice mit einer anderen Technologie erstellt werden kann, da bereits alle vorbereitenden Maßnahmen, wie Modellgenerierung aus den Schemadateien, getroffen wurden. 
...
Auf die vorhandene Implementierung der Business Logik und der Modelle der Schemata lässt sich sehr einfach ein weiterer \gls{Webservice} basierend auf SOAP implementieren. Das bedeutet, dass die oberste Schicht ausgetauscht werden kann. Es ist möglich einen RESTful \gls{Webservice} oder einen SOAP-\gls{Webservice} zuerstellen. 
