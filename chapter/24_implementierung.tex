...
Die \LaTeX Quelltexte dieser Arbeit sind bei einem frei verfügbaren \glslink{VCS}{Versionskontrollsystemanbieter} untergebracht. Das Repository kann mit folgendem Befehl geklont werden (\enquote{username} durch Benutzernamen beim Anbieter Bitbucket ersetzen):
...
Klonen bedeutet bei verteilten \glslink{VCS}{Versionskontrollsystemen}, das gesamte Repository zu kopieren. Es kann dann auf diesem Repository lokal gearbeitet werden. 
...
Alle weiteren Testfälle und Ergebnisse sowie die Beschreibung einiger Testtools zum Ausführen der manuellen Tests finden sich in \autoref{anh:testszenarios}.
...
Somit ergibt sich als aktuelle Gesamt-\gls{URL} für den \gls{Webservice} der Applikation \\~
...
\section{Query-Verarbeitung}\label{sec:query_vearbeitung}\index{Query}
...
\subsubsection{Benötigte XSD-Dateien}\index{XSD}
...
\lstinline[basicstyle=\ttfamily\small\mdseries]{xjc -b binding.xjb -d gen-src query.xsd}
...
\section{Transformation und Abfrage der PLIB-Prozeduren}\index{PLIB-Prozeduren}\index{Oracle Prozeduren}\label{sec:abfrage_plib_prozeduren}\index{Query Verarbeitung}
...
\subsection{Problemstellung - Externer Identifier}\label{sec:problemidentifier}\index{IRDI}
...
\begin{table}[!hbt]\vspace{1ex}\centering\scriptsize\begin{tabular}{p{3cm}p{10cm}}
...
Die genannten Prozeduren liefern die Daten der Eigenschaften eines spezifizierten \glslink{item}{Teiles} nach \gls{IRDI}. Die Liste der zurückgegebenen Daten scheint auf den ersten Blick ausreichend, denn man erhält die folgenden Eigenschaften der Prozedur: \\~ 
\linebreak
\linebreak
...
\subsection{Problemstellung - RECORD-Types als Argumente werden nicht von Java-JDBC unterstützt}\index{JDBC}\index{Record-Types}
...
Wenngleich die Abfrage zum Ergebnis führt, so ist sie sehr komplex. Daher sollte wie geplant die Abfragekomplexität in der Prozedur gekapselt sein. Damit sind wir gleich beim zweiten Lösungsansatz. Hierbei müssen die Prozeduren angepasst und als Rückgabetyp ein OBJECT-Typ benutzt werden. \index{Object-Type}
...
Für mehr Informationen zur Implementierung und den Beispielquellcode des \gls{SOAP}-\glspl{Webservice} siehe \autoref{kap:anhang_soap_webservice}. 
